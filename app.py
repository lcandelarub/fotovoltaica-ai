...
# --- INTERFAZ DE LA APP ---
st.title("🔆 Chat de Fotovoltaica AI (Modo Debug)")
...
def extract_text_from_pdf(pdf_file):
    ...
    with pdfplumber.open(pdf_file) as pdf:
        for page in pdf.pages:
            ...
            if page_text:
                # Dividimos en fragmentos de 300 palabras aprox para mejorar la búsqueda
                words = page_text.split()
                for i in range(0, len(words), 50):
                    fragment = " ".join(words[i:i+50])  # Fragmentos de 50 palabras
...
# --- MOSTRAR CONTENIDO DE knowledge.json PARA DEBUG ---
st.sidebar.subheader("📂 Contenido almacenado en la IA")
with open(knowledge_file, "r") as f:
    knowledge_data = json.load(f)

st.sidebar.write(knowledge_data)
...
if query:
    if not knowledge_data:
        ...
        for doc in knowledge_data.values():
            for fragment in doc:
                if any(word in fragment.lower() for word in query_words):
